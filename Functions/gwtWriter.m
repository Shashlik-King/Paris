...
%% Function generates filelist and writes .gwt's to project folder
    ...
    for j=1:NoSteps
        ...
        fprintf(fileID,'%10.4f %9.4f %9.4f %9.4f\r\n',[0 0 0 0]');
        fprintf(fileID,'%8.2f%10.3f%10.2f %7.3f %7.3f %7.3f %7.3f %7.3f %7.2f %7.4f %4.2f\r\n',[0 0 0 SRD.(loc{locLoop,1}).Soil.QuakeDamp(1,1) SRD.(loc{locLoop,1}).Soil.QuakeDamp(1,2) SRD.(loc{locLoop,1}).Soil.QuakeDamp(1,3) SRD.(loc{locLoop,1}).Soil.QuakeDamp(1,4) SRD.(loc{locLoop,1}).Soil.Setup SRD.(loc{locLoop,1}).Soil.LimDist(1) 0 SRD.(loc{locLoop,1}).gwtPile(size(SRD.(loc{locLoop,1}).gwtPile,1),2)]');
        i = locLoop;
