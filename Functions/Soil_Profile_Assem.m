...
SoilTableRaw = cell(size(CPTProfile,1)*2,12);


for i = 1:size(CPTProfile,1)
    for j = 1:2           %Useful PNGI
        SoilTableRaw((i-1)*2+j,:) = num2cell([CPTProfile{i,j}, NaN, NaN, NaN, CPTProfile{i,3+j}, CPTProfile{i,5+j}, NaN, NaN,NaN,NaN,NaN,NaN]);
...
for i=2:length(z)
    ...
    for ii=1:size(DepthSRD,1)
        if (z(i)+z(i-1))/2>= DepthSRD(ii,1) && (z(i)+z(i-1))/2 < DepthSRD(ii,2)           % to avoid confusion, middle of each sub layer should be inside of the General layering
            ...
            SoilTable(i,12)=SoilProfile_1(ii,10);
            SoilTable(i,13)=SoilProfile_1(ii,11);
            SoilTable(i,14)=SoilProfile_1(ii,12);
            %%%%Assiging the type of the soil
            
            if contains(lower(SRD_profile(ii,3)),'sand')
                SoilTable(i,3)=num2cell(1);      % Cohesionless equal to 1
            elseif  contains(lower(SRD_profile(ii,3)),'glauconite')
                SoilTable(i,3)=num2cell(4);      %sand containing glauconte soil equal to 3
            elseif   strcmp(lower(SRD_profile(ii,3)),'clay')
                SoilTable(i,3)=num2cell(2);      %Cohesive soil equal to 2
            elseif   strcmp(lower(SRD_profile(ii,3)),'clay_L_PI')
                SoilTable(i,3)=num2cell(2);      %Cohesive soil with low to meduim plasticity equal to 2 (Alm&Hamre 2018)
            elseif   strcmp(lower(SRD_profile(ii,3)),'clay_H_PI')
                SoilTable(i,3)=num2cell(3);      %Cohesive soil with High plasticity equal to 3 (Alm&Hamre 2018)
            elseif   contains(lower(SRD_profile(ii,3)),'rock')
                SoilTable(i,3)=num2cell(5);      %Rock equal to 5 (Stevens)
            end
            %%%%%%%%%%
...
SoilTable(1,10)=SoilProfile_1(1,8);    % Assigning su sensitivity (Jones)
SoilTable(1,11)=SoilProfile_1(1,9);    % Assigning  yield stress ratio (Jones)
SoilTable(1,12)=SoilProfile_1(1,10);    % Assigning su (Stevens)
SoilTable(1,13)=SoilProfile_1(1,11);    % Assigning OCR (Stevens)
SoilTable(1,14)=SoilProfile_1(1,12);    % Assigning UCS (Stevens and Puech)

%%%%Assiging the type of the soil
if contains(lower(SRD_profile(ii,3)),'sand')
    SoilTable(1,3)=num2cell(1);      % Cohesionless equal to 1
elseif  contains(lower(SRD_profile(ii,3)),'glauconite')
    SoilTable(1,3)=num2cell(4);      %sand containing glauconte soil equal to 3
elseif   strcmp(lower(SRD_profile(ii,3)),'clay')
    SoilTable(1,3)=num2cell(2);      %Cohesive soil equal to 2
elseif   strcmp(lower(SRD_profile(ii,3)),'clay_L_PI')
    SoilTable(1,3)=num2cell(2);      %Cohesive soil with low to meduim plasticity equal to 2 (Alm&Hamre 2018)
elseif   strcmp(lower(SRD_profile(ii,3)),'clay_H_PI')
    SoilTable(1,3)=num2cell(3);      %Cohesive soil with High plasticity equal to 3 (Alm&Hamre 2018)
elseif   contains(lower(SRD_profile(ii,3)),'rock')
    SoilTable(1,3)=num2cell(5);      %Rock equal to 5 (Stevens)
end
%%%%%%%%%%
