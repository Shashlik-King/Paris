...
if phi.tip(i)-5 <= 0 || phi.shaft(i)-5 <= 0
...
sigma_r_i   = 0.031*CPT.shaft(i,1)*(h_R_ratio^(-0.481*((D_tw_ratio)^0.145)));

if z_D(j) >= z(i)
    fs = 0.5*sigma_r_i*tan((phi.shaft(i))*pi/180);   %Friction is applied only outside 
else 
    fs = NaN;   
end

fsi         = 0.5*0.031*CPT.shaft(i,1)*(6^(-0.481*((D_tw_ratio)^0.145)))*tan((phi.shaft(i))*pi/180);                   % Friction only outside
fsres       = 0.5*0.031*CPT.shaft(i,1)*((z_D(end)/R_eqauv)^(-0.481*((D_tw_ratio)^0.145)))*tan((phi.shaft(i))*pi/180); 
qt          = 0.6*CPT.tip(i,1);
