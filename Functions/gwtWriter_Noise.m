...
% % % %    % for soilMethodLoop = 1:soilMethod   % Loop around BE and UB if switch turned on
        % Loop over number of steps/file for current location to create all files
       
            Index(j) = sum(SRD.(loc{i,1}).Soil.z_D(j)>=SRD.(loc{i,1}).Soil.z); % Define index for soil properties
            %filelist.(soil_method_profile{soilMethodLoop}){(i-1)*NoSteps+j,1} =strcat(pwd,'\',Settings.Analysis{A.CALC},'\',loc{i,1},SRDmethod,'_',soil_method_profile{soilMethodLoop},num2str(j));
%              if i == 1 && j == 1
            filelist = {strcat(pwd,'\',Settings.Analysis{A.CALC},'\',loc{i,1},SimulationLable,'_',num2str(j))};
%              else
%             filelist = [filelist; strcat(pwd,'\',Settings.Analysis{A.CALC},'\',loc{i,1},SimulationLable,'_',num2str(j))];
%             end
...
%     SRD.(loc{i,1}).Indexfilelist(2) = size(filelist.(soil_method_profile{soilMethodLoop}),1);
