function [fsi, fsres, qt, k, fs, SkinQuake, ToeQuake, SkinDamping, ToeDamping] = Model_Alm_Hamre(j,i,T,sigv,Pa,CPT,z,z_D,~,~,phi,Damping_Table, Glauconite_Rf_Multiplier,loc)
...
elseif T.tip(i) == 4  % Glauconite
    ...
    error(['Soil not defined for ' loc{1} ' in AlmHamre function' ])
end

%% Shaft friction calculation
if T.shaft(i) == 1   %SAND
    if phi.shaft(i) <= 0
        error('Wrong input of phi for sand in AlmHamre - phi-5 used for calculation is negative ')
    end
