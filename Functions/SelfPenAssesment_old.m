...
Assem_Weight=Settings.AssemWeight(A.Analysis);
    ...
    [SRD.(loc{locLoop}).SWPpileWtIdx,  SRD.(loc{locLoop}).SWPpileWtDepth,  SRD.(loc{locLoop}).SkinSWP,   SRD.(loc{locLoop}).TipSWP,    SRD.(loc{locLoop}).totalRes]=SelfPenDepth(Z_D,z,ToeResistance,Shaft_res_long,Piameter,PileWeight);
    
    [SRD.(loc{locLoop}).SWPTotalWtIdx, SRD.(loc{locLoop}).SWPTotalWtDepth, SRD.(loc{locLoop}).SkinSWP ,  SRD.(loc{locLoop}).TipSWP,    SRD.(loc{locLoop}).totalRes]=SelfPenDepth(Z_D,z,ToeResistance,Shaft_res_long,Piameter,TotalWeight);   
    
 
        
    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 function [IdxSWP,DepthSWP,total_Shaft, TipRes,  totalRes]=SelfPenDepth(Z_D,z,ToeResistance,Shaft_res_long,Piameter,Weight)
       ...
       for j=1:length(Z_D)
        ...
        Tip_ActiveSoil=ToeResistance(find(Z_D(j)>=z));
        Skin_ActiveSoil=Shaft_res_long(find(Z_D(j)>=z));
